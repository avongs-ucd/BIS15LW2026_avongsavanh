...
polar_bears %>% 
  distinct(mating) 
```
Here we see that "unknown" is the alternative representation of NA in this dataset.

```{r}
polar_bears <- polar_bears %>% #going back to the original data
  replace_with_na(replace = list(mating = "unknown", 
                                 denning = "unknown")) #found unknown in denning as well through inspection
```
...
polar_bears %>%
  ...
  count(sex)
```
The sampling is not even as there are more much more females than males.

```{r}
polar_bears %>% 
  ggplot(mapping = aes(x = popn, fill = sex)) +
  geom_bar(position = "dodge") + 
  labs(title = "Number of Male and Female Polar Bears by Population",
      x = "Population",
      y = "Number of Polar Bears")
```
...
```{r}
polar_bears %>% #Make a line plot(s) to visualize sampling by year for both populations.
  filter(popn == "SB") %>% 
  select(year) %>% 
  count(year) %>% 
  ...
  labs(title = "Polar Bear Sampling by Year - SB Population",
      x = "Year",
       y = "Number of Polar Bears Sampled")
```
```{r}
polar_bears %>% #Make a line plot(s) to visualize sampling by year for both populations.
  filter(popn == "CS") %>% 
  ...
  geom_point(shape = 5) + 
  theme(axis.text.x = element_text(angle = 60, hjust = 1)) + 
  labs(title = "Polar Bear Sampling by Year",
      x = "Year",
       y = "Number of Polar Bears Sampled")
```
...
```{r}
polar_bears %>% 
  summarize(urea_nitrogen_min = min(urea_nitrogen),
         urea_nitrogen_med = median(urea_nitrogen),
         urea_nitrogen_mean = mean(urea_nitrogen),
         urea_nitrogen_max = max(urea_nitrogen))
```
```{r}
polar_bears %>% 
  summarize(creatinine_min = min(creatinine),
         creatinine_med = median(creatinine),
...
polar_bears %>% #Make a line plot(s) to visualize sampling by year for both populations.
  ...
  geom_point(shape = 5) + 
  theme(axis.text.x = element_text(angle = 60, hjust = 1)) + 
  labs(title = "Fasting Polar Bear Sampling by Year - SB Population",
      x = "Year",
       y = "Number of Fasting Polar Bears Sampled")
```
```{r}
polar_bears %>% #Make a line plot(s) to visualize sampling by year for both populations.
  filter(popn == "CS", fasting == "yes") %>% 
  select(year) %>% 
  count(year) %>% 
  ggplot(aes(x = year, y = n)) +
  geom_line(color = "red") +
  geom_point(shape = 5) + 
  theme(axis.text.x = element_text(angle = 60, hjust = 1)) + 
  labs(title = "Fasting Polar Bear Sampling by Year - CS Population",
      x = "Year",
       y = "Number of Fasting Polar Bears Sampled")
```
...
```{r}
lat_box <- c(lat_min, lat_max)
long_box <- c(long_min, long_max)
  bbox <- make_bbox(long_box, lat_box, f = 0.03)
```

```{r}
polar_bears_map <- get_stadiamap(bbox, maptype = "stamen_terrain", zoom = 7)
```

```{r}
ggmap(polar_bears_map) +
  geom_point(data = polar_bears, aes(long, lat, color = popn), size = 0.5) +
  labs(x = "Longitude",
       y = "Latitude",
       title = "Polar Bear Distribution") 
```
...
ui <- dashboardPage(
  ...
  dashboardHeader(title = "un_creat_ratio vs. variable of interest"), 
  ...
  dashboardBody(
  helpText("Spring fasting behavior in a marine apex predator provides an index of ecosystem productivity: Global Change Biology, v. 24, no. 1, p. 410–423, https://doi.org/10.1111/gcb.13933."),
...
server <- function(input, output, session) {
  ...
  output$plot <- renderPlot({
    ...
    polar_bears %>% 
      ggplot(aes(x = un_creat_ratio, y = .data[[input$y]], fill = popn)) +
      geom_boxplot(alpha = 0.8) +
      labs(title = "un_creat_ratio vs. variable of interest", 
           x = "un_creat_ratio", 
           y = input$y) +
      theme_minimal()
  })
