---
title: "Midterm 2 W26 - Corrections"
date: "`r Sys.Date()`"
output:
  html_document:
    theme: spacelab
...
The error made above was that the count was only by sex and not by population and sex. The corrected code is below: 
```{r}
polar_bears %>%
  select(sex, popn) %>%
  count(sex, popn)
```
The sampling is not even as there are more much more females than males between the two populations.
...
The error made above was that the line plots did not have lines plotted. The corrected code is below:
```{r}
#polar_bears <- polar_bears %>% #same code as before
#  separate(capture_date, into = c("year", "month", "day"), sep = "-") 
#polar_bears
```
...
polar_bears %>% 
  ...
  geom_point(shape = 5) + 
  theme(axis.text.x = element_text(angle = 60, hjust = 1)) + 
  labs(title = "Polar Bear Sampling by Year - CS Population",
      x = "Year",
       y = "Number of Polar Bears Sampled")
```
