...
```{r}
name <- c("Mt_Everest","K2","Kangchenjunga","Lhotse","Makalu")
height_m <- c(8848, 8611, 8586,8516,8485) 
height_ft <- c(29031.7,28251,28169,27940,27838)
```

**2. Combine these vectors into a data frame called `mountains`.**
```{r}
mountains <- tibble(name,height_m,height_ft)
mountains
```

**3. What is the mean height of the mountains in feet?**
```{r}
mheight_ft <- mean(mountains$height_ft)
mheight_ft
```
...
```{r}
first_climbed <- c(1953, 1954, 1955, 156, 1955)
```

```{r}
mountains <- tibble(name,height_m,height_ft,first_climbed) 
mountains
```
...
```{r}
summits <- c(12884, 800, 250, 900, 560)
mountains <- tibble(name,height_m,height_ft,first_climbed,summits) 
mountains
```
...
```{r}
fatalities <- c(340,100,60,22,96)
mountains <- tibble(name,height_m,height_ft,first_climbed,summits,fatalities) 
mountains
```
...
```
```{r}
mountains <- tibble(name,height_m,height_ft,first_climbed,summits,fatalities, fatality_rate) 
mountains
```
