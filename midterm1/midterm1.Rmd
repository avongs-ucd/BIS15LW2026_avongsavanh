...
Link to repository: https://github.com/avongs-ucd/BIS15LW2026_avongsavanh
...
```
Sampling is not equal across all islands as islands like Cuba and Hipaniola have much more sampling than islands like Jamacia and Puerto Rico.
...
anolis %>%
  ggplot(mapping = aes(x = habitat)) +
    ...
    labs(title = "Number of Anole Species Sampled Across Each Habitat Type",
      x = "Habitat Type",
      y = "Number of Anole Species")  
```
Sampling is not equal across habitat types as some habitat types like trunk-ground and grass-bush have more sampling than those like trunk-crown and twig. 
...
anolis %>%
  ggplot(mapping = aes(x = habitat, y = hindlimb_length_mm)) +
    ...
    labs(title = "Hindlimb Length (mm) Across Each Habitat Type",
      x = "Habitat Type",
      y = "Hindlimb Length (mm)")  
```
The range of hindlimb length definitely varies across each of the different habitat types as we can see different IQR ranges for each of the habitat types. 
...
anolis %>%
  ggplot(mapping = aes(x = habitat, y = ratio_of_hindlimb_to_body)) + 
    geom_boxplot(mapping = aes(fill = habitat)) +
    labs(title = "Ratio of Hindlimb to Body Length Across Each Habitat Type",
      x = "Habitat Type",
      y = "Ratio of Hindlimb to Body Length")  
```
This new plot differs from the one made in #10 as we see each of the ranges for each habitat being slightly different from the previous plot (in appearance, the values of the y axis are much different as the new plot looks at a ratio instead of a measured value), but most notably we see a difference in the ranges entirely for the species in the grass-bush habitat as the range of the hindlimb length was in a lower range compared to the other habitats and the range of the ratio of hindlimb-to-body-length of the species in the grass-bush habitat is in a higher range compared to the other habitats. . 
...
anolis %>%
  ggplot(mapping = aes(x = body_length_mm, y = tail_length_mm)) + 
    ...
    labs(title = "Body Length (mm) vs. Tail Length (mm)",
      ...
      y = "Tail Length (mm)")  
```
This plot suggests that there is a positive relationship between tail length and body length (in that it predicts that if there is a long body, there will be a long tail). For the lizards in the grass-bush habitat, I notice that there are longer tails for much shorter body lengths when comparing this to all of the species recorded. If there was another plot for just the lizards in the grass-bush habitat, the slope would be greater (more positive relationship), also indicating that the longer the body length, the longer the tail length (even more so than the plot shown above). 
...
anolis %>%
  ...
  summarize(toepad_lamellae_count_m = mean(toepad_lamellae_count),
            total = n())
```
...
anolis_tc %>% 
  ...
  labs(title = "Number of Toepad Lamellae by Island for Trunk-Crown Species Only",
      x = "Island Name",
      y = "Number of Toepad Lamellae") 
```
This is not consistent across all islands as the number of toepad lamellae for trunk-crown species only is different for each of the islands. The ranges for the number of toepad lamellae for trunk-crown species only are much different for each of the islands as there is little to no overlap between the heights that the ranges (for number of toepad lamellae) of each island cover. 
